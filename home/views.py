...
def import_stock(request):
    ...
    if request.method == "POST":
        ...
        stock_data = stock_log3(user_request=import_useX, user_handler=str(request.user), date_input=input_date, date_sys=sys_date, item_name=import_item,
                                item_count=Stock, admin_name=str(request.user), admin_date=sys_date, action_status="import action", info_7=comment, info_1=Document)
        stock_data.save()
        if stock_data.save():
            print("saved")
            return render(request, "import_page.html", {"info_page": "Done", "import_name": import_user,
                                                        "items": items, "import_type": type_name, "user_sys": request.user, "role": role, "department": department})
        else:
            return render(request, "import_page.html", {"import_name": import_user,
                                                        "items": items, "import_type": type_name, "user_sys": request.user, "role": role, "department": department, "role_control": role_control,
                                                        "ADD": 1})
    else:
        return render(request, "import_page.html", {"import_name": import_user, "items": items, "import_type": type_name, "user_sys": request.user, "role": role, "department": department, "role_control": role_control})
...
def export(request):
    ...
    if request.method == "POST":
        ...
        stock_data = stock_log3(user_request=str(request.user), department=str(department), user_handler="1st approval", date_input=input_date, date_sys=sys_date, item_name=import_item, item_count=Stock, admin_name=str(request.user), admin_date=sys_date, action_status="export request", info_7=comment, info_2=str(role),
        ...
        return render(request, "export_page.html", {"import_name": import_user, "items": items, "import_type": type_name, "user_sys": request.user, "role": role, "department": department,
                                                    "role_control": role_control})
...
def add_user(request):
    ...
    if request.method == "POST" and request.POST['btn'] == "save":
        ...
        try:
            ...
            return render(request, "add_user.html", {"info": "failed " + str(e), "username": username, "Password": password, "email": email, "Dep": depart})

    if request.method == "POST" and request.POST['btn'] == "find":
        username = request.POST['username']
        data_ = User.objects.all().filter(username=str(username))
        return render(request, "add_user.html", {"username": data_.username, "Password": data_.password, "email": data_.first_name, "Dep": data_.last_name})
...
def request_view(request):
    ...
    if request.method == "POST":
        ...
        if action == "Export":
            ...
            report_info = report_log(report_user=str(request.user), report_date=str(
                datetime.now()), report_name=file_name + ".xlsx")
            report_info.save()
            return render(request, "request_page.html", {"requests": request_data, "role_control": role_control,
                                                         "sd": sd, "ed": ed, "id": id_actoin, "user_req_data": req_user, "import_item_n": item_import,
...
def import_req(request):
    ...
    all_data = stock_log3.objects.filter(
        action_status="import action").order_by('-date_sys')
    if request.method == "POST":
...
def export_req(request):
    ...
    all_data = stock_log3.objects.filter(
        action_status="export request", user_handler="1st approval").order_by('-date_sys')
    if request.method == "POST":
        id_value = request.POST['ID']
        I = request.POST['item_n']
        C = request.POST['item_c']
        ...
        if action_value == "Export Rejected 1st":
            stock_log3.objects.filter(action_id=id_value).update(action_status=action_value,
                                                                 admin_date=str(datetime.now())[
                                                                     :19], date_sys=str(datetime.now())[:19],
                                                                 admin_name=str(
                                                                     request.user),
                                                                 user_handler="")
        else:
...
def export_req_2(request):
    ...
    all_data = stock_log3.objects.filter(
        action_status="export 1st aprroved", user_handler="2nd approval").order_by('-date_sys')
    if request.method == "POST":
        id_value = request.POST['ID']
        I = request.POST['item_n']
        C = request.POST['item_c']
...
def stock_req(request):
    ...
    for x in item_array:
        stock_data = stock_log3.objects.filter(item_name=str(
            x), action_status__in=("In Stock", "Export Approved"))
        total = 0
        for z in stock_data:
            total += int(z.item_count)
        item_stock.append(total)

        stock_data = stock_log3.objects.filter(
            item_name=str(x), action_status=("Export Approved"))
        consumed = 0
        for z in stock_data:
        ...
        stock_data = stock_log3.objects.filter(
            item_name=str(x), action_status=("In Stock"))
        import_c = 0
        for z in stock_data:
            import_c += int(z.item_count)
        item_import.append(import_c)

    list_zip = zip(item_id, item_type, item_array, item_stock,
                   item_consumed, item_import, first_stock)

    return render(request, "stock_req.html", {"list_zip": list_zip, "role_control": role_control, "item_req": item_req})
...
def report_page(request):
    ...
    all_data = report_log.objects.filter(
        report_user=str(request.user)).order_by('-action_id')
...
def generate_report_admin(data, name):
    workbook = xlsxwriter.Workbook(
        "assets\\report_export\\" + str(name) + ".xlsx")
    ...
    list_zip = zip(item_type, item_id, item_array, item_stock,
                   item_consumed, item_import, first_stock)
